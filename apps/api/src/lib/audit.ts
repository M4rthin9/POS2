...
export function auditStatement(c: Ctx, e: AuditEntry): D1PreparedStatement {
  ...
  return c.env.DB.prepare(
    `INSERT INTO audit_log (actor_user_id, action, entity, entity_id, before_json, after_json, reason, ip, created_at)
     VALUES (?,?,?,?,?,?,?,?, datetime('now'))`,
  ).bind(user?.id ?? null, e.action, e.entity, e.entity_id ?? null, json(e.before), json(e.after), e.reason ?? null, ip);
}

...
// Ported from the CIDA POS prototype promptpay.js
// Credit: https://github.com/dtinth/promptpay-qr
// Generates the EMVCo PromptPay payload entirely client-side (works offline).
...
function serialize(xs: (string | null | false | undefined)[]): string {
...
// Mirrors the prototype: phone numbers are converted to 66 + number (leading 0
// stripped), then zero-padded to 13 digits on the right.
function formatTarget(id: string): string {
  let t = sanitizeTarget(id);
  if (t.length <= 12) {
    if (t.startsWith('0')) t = t.substring(1);
    t = '66' + t;
    if (t.length < 13) t = t + '0'.repeat(13 - t.length);
  }
  return t;
}
...
export interface PromptPayPayload {
  target: string;
  targetType: 'phone' | 'taxid' | 'ewallet';
  amount: number | null;
  payload: string;
}

/**
 * Generate the EMVCo PromptPay payload (EMV QRCPS merchant-presented mode).
 * - mobile number (<= 12 digits) -> 66 + number, zero-padded to 13, field 01
 * - TAX ID (13 digits)           -> as-is, field 02
 * - e-Wallet ID (15+ digits)     -> as-is, field 03
 * Amount > 0 produces a dynamic QR; otherwise a static QR.
 */
export function generatePayload(target: string, amount?: number): PromptPayPayload {
  const sanitized = sanitizeTarget(target);
  const amt = amount && amount > 0 ? Number(amount) : null;
  const targetType: 'phone' | 'taxid' | 'ewallet' =
    sanitized.length >= 15 ? 'ewallet' : sanitized.length >= 13 ? 'taxid' : 'phone';
  const botId =
    targetType === 'ewallet'
